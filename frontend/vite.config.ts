import { defineConfig } from "vite";
import react from "@vitejs/plugin-react";
import { VitePWA } from "vite-plugin-pwa";

// Base path for deployment behind a path-prefix ALB rule.
// Set VITE_BASE_PATH="/scanner/sc1" in deploy/.env.prod (no trailing slash).
// Defaults to "" (root) for local dev.
const basePath = process.env.VITE_BASE_PATH || "";
const base = basePath ? `${basePath}/` : "/";

export default defineConfig({
  base,
  plugins: [
    react(),
    VitePWA({
      registerType: "autoUpdate",
      includeAssets: ["favicon.ico", "favicon.png", "icons/*.png"],
      strategies: "injectManifest",
      srcDir: "src",
      filename: "sw.ts",
      injectManifest: {
        globPatterns: ["**/*.{js,css,html,ico,png,svg,woff2}"],
      },
      devOptions: { enabled: true, type: "module" },
      manifest: {
        name: "BundleBox",
        short_name: "BundleBox",
        description: "Smart room inventory in minutes",
        theme_color: "#ffffff",
        background_color: "#ffffff",
        display: "standalone",
        orientation: "portrait",
        start_url: base,
        icons: [
          { src: `${base}icons/icon-192.png`, sizes: "192x192", type: "image/png" },
          { src: `${base}icons/icon-512.png`, sizes: "512x512", type: "image/png" },
          {
            src: `${base}icons/icon-512.png`,
            sizes: "512x512",
            type: "image/png",
            purpose: "maskable",
          },
        ],
      },
    }),
  ],
  server: {
    port: 5173,
    host: true,
    allowedHosts: [".trycloudflare.com"],
    proxy: {
      "/api": { target: "http://localhost:8000", changeOrigin: true },
      "/api/v1/ws": { target: "ws://localhost:8000", ws: true },
    },
  },
});
