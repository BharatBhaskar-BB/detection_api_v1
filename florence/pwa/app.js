// ── BundleBox PWA — Live Detection + AI Inventory ──────────────────────
...
let capturedFrames = []; // base64 frames for AI inventory batch
const CAPTURE_INTERVAL = 2000; // capture a key frame every 2s for inventory
...
// Check if landing was already completed (returning user → go to home)
...
async function checkServer() {
    ...
    const lText = document.getElementById('l-status-text');
    const lDot = lStatus ? lStatus.querySelector('.status-dot-sm') : null;
    const lBtn = document.getElementById('l-start');
    try {
        ...
        // Update landing page
        if (lDot) { lDot.className = 'status-dot-sm on'; }
        if (lText) { lText.textContent = 'AI Ready · Connected'; }
        ...
        // Update landing page
        if (lDot) { lDot.className = 'status-dot-sm off'; }
        if (lText) { lText.textContent = 'Server Offline'; }
...
async function startScan() {
    ...
    if (renderMode === 'spotter') {
        ...
        startOverlaySync();
        document.getElementById('s-perimeter').classList.add('active');
    } else {
        document.getElementById('s-perimeter').classList.add('active');
    }
...
function sendFrame() {
    ...
    // Capture key frame for inventory (always full-res)
    const now = Date.now();
...
function drawSpotterDots(sCtx, bboxes, scaleX, scaleY) {
    const dotR = 4;
    ...
    for (let i = 0; i < bboxes.length; i++) {
        ...
        const pulse = ((now + i * 200) % 1500) / 1500; // stagger per object
        const ringR = dotR + pulse * 10;
        const ringAlpha = 0.5 * (1 - pulse);
        sCtx.beginPath();
        sCtx.arc(cx, cy, ringR, 0, Math.PI * 2);
        sCtx.strokeStyle = `rgba(16,185,129,${ringAlpha})`;
        sCtx.lineWidth = 1.5;
        sCtx.stroke();
...
function spotterRenderLoop() {
    ...
    if (lastDetection && lastDetection.bboxes.length > 0) {
        ...
        drawSpotterDots(spotterCtx, bboxes, lastDetection.scaleX, lastDetection.scaleY);
    }
...
function stopSpotterMode() {
    ...
    ['s-spotter-sweep', 's-spotter-corners', 's-spotter-sparkles',
        's-spotter-badge', 's-spotter-toasts', 's-perimeter'].forEach(id => {
...
function finishScan() {
    ...
    const scanline = document.getElementById('s-scanline');
    if (scanline) scanline.classList.remove('active');
    const perimeter = document.getElementById('s-perimeter');
    if (perimeter) perimeter.classList.remove('active');
    lastDetection = null;
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    stopSpotterMode();
    ...
    // Send key frames to AI for inventory
    runGeminiBatch();
}
...
function showInventory(data) {
    ...
    // Update stats with AI info
    const statsEl = document.getElementById('r-stats');
    statsEl.innerHTML += `<br><br>AI Inventory Analysis<br>` +
...
async function uploadArtifacts() {
    ...
    try {
        ...
        // 2. Key frames sent for AI analysis
        let frames = capturedFrames;
        if (frames.length > 15) {
            const step = frames.length / 15;
            frames = Array.from({ length: 15 }, (_, i) => capturedFrames[Math.floor(i * step)]);
